import { login, logout, getNavList, getNewToken } from './api'
...
const actions = {
  ...
  Login({ commit }, userInfo) {
    ...
    return new Promise((resolve, reject) => {
      login(username, userInfo.password).then(response => {
        ...
        commit('user/setName', data.name, { root: true })
        commit('user/setAvatar', data.avatar, { root: true })
        resolve()
      }).catch(error => {
        reject(error)
      })
  ...
  relogin({ dispatch, commit, state }) {
    return new Promise((resolve) => {
      ...
      commit('user/setAvatar', decodeURIComponent(Cookies.get('userAvatar')), { root: true })
      resolve()
    })
  },
